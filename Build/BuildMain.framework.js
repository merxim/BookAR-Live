...
var ASM_CONSTS = {
  4126468: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4126523: function($0) {performance.now = function() { return $0; };},  
 4126571: function($0) {performance.now = function() { return $0; };},  
 4126619: function() {performance.now = Module['emscripten_get_now_backup'];},  
 4126674: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4126735: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4126799: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _IsWebGLwTrackerReady()
      {
          return window.wTracker != null;
      }

  function _IsWebcamPermissionGranted()
      {
  ...
  function _SetWebGLARCameraSettings(settings)
  	{
      	window.arCamera.setARCameraSettings(UTF8ToString(settings));
      }

  function _SetWebGLwTrackerSettings(settings)
  	{
      	window.wTracker.setTrackerSettings(UTF8ToString(settings),"1.4.11.590507");
      }

  function _ShowConfirmUrl(url)
      {
          window.ShowConfirmUrl(UTF8ToString(url));
      }

  function _ShowWebGLScreenshot(dataUrl)
      {
          window.ShowScreenshot(UTF8ToString(dataUrl));
      }

  function _StartWebGLwTracker(name)
  	{
          if(!window.wTracker){
  ...
  function _StopWebGLwTracker()
  	{
      	window.wTracker.stopTracker();
      }

  function _WebGLFlipCamera(){
          window.FlipCam();
      }

  function _WebGLGetCameraTexture(textureId){
  ...
  function _WebGLGetGPSPosition(){
          window.wTracker.getGPSPosition();
      }

  function _WebGLGetVideoDims()
      {
  ...
  function _WebGLIsCameraFlipped(){
          return window.arCamera.videoCanvas.style.transform == "scaleX(-1)";
      }

  function _WebGLIsCameraStarted()
      {
          if(!window.arCamera){
  ...
  function _WebGLPauseCamera()
  	{
      	window.arCamera.pauseCamera();
      }

  function _WebGLPlaceOrigin(camPosStr)
      {
          window.wTracker.placeOrigin(UTF8ToString(camPosStr));
      }

  function _WebGLResetOrigin()
      {
          window.wTracker.resetOrigin();
      }

  function _WebGLStartCamera()
      {
          window.StartWebcam();
      }

  function _WebGLSubscribeToGPSPositionUpdates(){
          window.wTracker.subscribeToGPSPositionChanges();
      }

  function _WebGLSubscribeVideoTexturePtr(textureId){
  ...
  function _WebGLSyncSSPos(vStr){
          window.wTracker.syncSSPos(UTF8ToString(vStr));
      }

  function _WebGLUnpauseCamera()
  	{
      	window.arCamera.unpauseCamera();
      }

  function _WebGLUnsubscribeToGPSPositionUpdates(){
...
var asmLibraryArg = {
  ...
  "IsWebGLwTrackerReady": _IsWebGLwTrackerReady,
  "IsWebcamPermissionGranted": _IsWebcamPermissionGranted,
  ...
  "SetWebGLARCameraSettings": _SetWebGLARCameraSettings,
  "SetWebGLwTrackerSettings": _SetWebGLwTrackerSettings,
  "ShowConfirmUrl": _ShowConfirmUrl,
  "ShowWebGLScreenshot": _ShowWebGLScreenshot,
  "StartWebGLwTracker": _StartWebGLwTracker,
  "StopWebGLwTracker": _StopWebGLwTracker,
  "WebGLFlipCamera": _WebGLFlipCamera,
  "WebGLGetCameraTexture": _WebGLGetCameraTexture,
  "WebGLGetGPSPosition": _WebGLGetGPSPosition,
  "WebGLGetVideoDims": _WebGLGetVideoDims,
  "WebGLIsCameraFlipped": _WebGLIsCameraFlipped,
  "WebGLIsCameraStarted": _WebGLIsCameraStarted,
  "WebGLPauseCamera": _WebGLPauseCamera,
  "WebGLPlaceOrigin": _WebGLPlaceOrigin,
  "WebGLResetOrigin": _WebGLResetOrigin,
  "WebGLStartCamera": _WebGLStartCamera,
  "WebGLSubscribeToGPSPositionUpdates": _WebGLSubscribeToGPSPositionUpdates,
  "WebGLSubscribeVideoTexturePtr": _WebGLSubscribeVideoTexturePtr,
  "WebGLSyncSSPos": _WebGLSyncSSPos,
  "WebGLUnpauseCamera": _WebGLUnpauseCamera,
  "WebGLUnsubscribeToGPSPositionUpdates": _WebGLUnsubscribeToGPSPositionUpdates,
